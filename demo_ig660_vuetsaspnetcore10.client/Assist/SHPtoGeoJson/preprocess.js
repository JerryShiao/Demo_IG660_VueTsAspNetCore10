...
SHPParser.prototype.parseShape = function(dv, idx, length) {
    ...
    case 15: // PolygonZ
            ...
            var minZ = dv.getFloat64(idx, true);
            var maxZ = dv.getFloat64(idx + 8, true);
            idx += 16; //minZ&maxZ
            for (i = 0; i < c.points.length; i++) {
                idx += 8;
            }
            var minM = dv.getFloat64(idx, true);
            var maxM = dv.getFloat64(idx + 8, true);
            idx += 16; //minM&maxM
            for (i = 0; i < c.points.length; i++) {
                idx += 8;
            }
...
DBFParser.prototype.parse = function(arrayBuffer,src,response,encoding) {
    ...
    idx += 2;
    o.bytesInRecord = dv.getInt16(idx, true); //10
    idx += 2;
    //reserved bytes
    idx += 2;
    ...
    while(responseHeader.length > 0) {
        ...
        var responseHeaderCH = 0;
        if (encodeURIComponent(responseHeader.slice(0, 10).replace(/\0/g, '')).match(/%[A-F\d]{2}/g) == null) {
            charString.push(responseHeader.slice(0, 10).replace(/\0/g, '')); 
        }
        else {
            responseHeaderCH = (encodeURIComponent(responseHeader.slice(0, 10).replace(/\0/g, '')).match(/%[A-F\d]{2}/g));// .length/3
            $.each(responseHeaderCH, function (key, val) {
                if (val == "%00") { responseHeaderCH.splice(key, 1) }
            })
            responseHeaderCH = responseHeaderCH.length / 3;
            if (encoding.match(/big5/i)) {
                charString.push(responseHeader.slice(0, 10 - responseHeaderCH).replace(/\0/g, '')); 
            }
            else {
                charString.push(responseHeader.slice(0, 10 - responseHeaderCH*2).replace(/\0/g, '')); 
            }
        }
        if (responseHeader.slice(0, 10).replace(/\0/g, '').length != encodeURIComponent(responseHeader.slice(0, 10).replace(/\0/g, '')).length) {
            if (encoding.match(/big5/i)) {
                responseHeader = responseHeader.slice(32 - responseHeaderCH, responseHeader.length);
            }
            else {
                responseHeader = responseHeader.slice(32 - (responseHeaderCH*2), responseHeader.length);
            }
        }
        else {
            responseHeader = responseHeader.slice(32, responseHeader.length);
