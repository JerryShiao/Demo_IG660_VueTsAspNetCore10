...
function onFileSelected(evt) {
    ...
    jszip.loadAsync(file).then((zip => {
        console.log('zip', zip)
        $.each(zip.files, function (fileName, file) {
            console.log('fileName', fileName)
            if (fileName.indexOf('.DXF') != -1 || fileName.indexOf('.dxf') != -1) {
                zip.file(fileName).async('blob').then(function (content) {
                    let file_ = new File([content], fileName, { type: 'image/x-dxf' });
                    onFileSelected_(file_)
                })
            }
        })
    }))
